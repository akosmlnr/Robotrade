...
## 4.1) Docker Usage (Recommended)

The easiest way to run this project is using Docker. We provide a comprehensive Docker setup with a Makefile for easy management.
...
2. **Train a model with default parameters:**
   ...
   make train
   # or
   docker run --rm -v $(PWD)/outputs:/app/outputs -v $(PWD)/models:/app/models -v $(PWD)/data:/app/data stock-prediction
   ```

3. **Train for different stocks:**
   ```bash
   make train-aapl    # Train for Apple stock
   make train-tsla    # Train for Tesla stock
   ```

4. **Run with custom parameters:**
   ```bash
   make run-custom TICKER=AAPL EPOCHS=50
   ```

5. **Start Jupyter notebook for interactive development:**
   ```bash
   make up-jupyter
   # Access at http://localhost:8888
   ```

6. **Run real-time predictions:**
   ```bash
   make run-realtime
   ```
...
make train-tsla             # Train for TSLA stock
...
make run-interactive        # Run container in interactive mode
...
- **stock-prediction**: Main training service
- **realtime-prediction**: Real-time prediction service
...
The Docker setup creates the following directories for data persistence:
- `outputs/`: Training outputs, plots, and results
...
1. **Permission Issues on Linux/macOS:**
   ```bash
   # Fix permission issues
   sudo chown -R $USER:$USER outputs/ models/ data/
   ```

2. **Out of Memory Errors:**
   ```bash
   # Reduce batch size in training
   make run-custom TICKER=GOOG EPOCHS=50 BATCH_SIZE=16
   ```

3. **GPU Support (Optional):**
   ```bash
   # For GPU support, modify docker-compose.yml to add:
   # deploy:
   #   resources:
   #     reservations:
   #       devices:
...
4. **Clean Up Resources:**
   ...
   make clean-outputs  # Clean only output files
   ```

### Docker Best Practices

- **Data Persistence**: All important data is stored in mounted volumes
- **Development**: Use `make up-jupyter` for interactive development
- **Production**: Use `make train` for automated training
- **Monitoring**: Use `make logs` to monitor training progress
- **Resource Management**: Use `make status` to check container status

## 4.2) Local Installation

If you prefer to run without Docker, download the source code and install the following packages:
